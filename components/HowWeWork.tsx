  ...
  return (
    <Section id="how-we-work" title={t["howWeWork.title"] as string}>
      <div className="space-y-6">
        {methodology && (
          <Card className="p-8">
            <p className="text-body text-text leading-relaxed max-w-3xl">{methodology}</p>
          </Card>
        )}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {steps.map((step, idx) => (
              <Card key={idx} elevated className="relative">
                <span className="absolute -top-2 -start-2 w-8 h-8 rounded-lg border border-accent/45 bg-accent/20 flex items-center justify-center text-body-sm font-semibold text-accent">
                  {idx + 1}
                </span>
